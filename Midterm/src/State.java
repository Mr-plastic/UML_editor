...
abstract class State  extends JLabel implements State_interface{
	protected static ArrayList<Base_Object> object_list = new ArrayList<Base_Object>();
	protected ArrayList<Base_Object> group_list = new ArrayList<Base_Object>();
...
class Select_state extends State{
	...
	public void Press_event(Canvas canvas,MouseEvent e){
		...
		if(group_list.size()>0) {
			for(int i = 0 ; i < group_list.size(); i++) { // refresh dragSelect_obj
				group_list.remove(i);
			}
		}
...
//----------------Class_object-----------------------------------------------------------------------------------------------------------		
		for(int i = object_list.size()-1 ; i >= 0; i--) {
			if(object_list.get(i).Object_selcet(press_point_X,press_point_Y,object_list.get(i).getX(),object_list.get(i).getY())) {
	...
	public void Release_event(Canvas canvas, MouseEvent e) {
		if(Drag_detect) {
			...
			if(object_list.get(i).ingroup == false &&object_list.get(i).getX() > Math.min(press_point_X,Drag_point_X) && object_list.get(i).getX() < Math.max(press_point_X,Drag_point_X) && object_list.get(i).getY() > Math.min(press_point_Y,Drag_point_Y) && object_list.get(i).getY() < Math.max(press_point_Y, Drag_point_Y)) {
...
class Assoc_state extends State{
	...
	protected Base_Object start_obj;
	protected Base_Object end_obj;
