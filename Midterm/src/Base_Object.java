	...
	Base_Object(Canvas cans,Icon img,int X,int Y,int Max_x,int Max_y,MouseEvent e){
		...
		if(img != null) {// class_Object && Case_object 
			...
			for(int i = 0 ; i < cans.Now_state.group_list.size(); i++) {
				cans.Now_state.group_list.get(i).ingroup = true;
				cans.Now_state.group_list.get(i).focus(cans.Now_state.group_list.get(i),new ImageIcon("./img/class.png"),new ImageIcon("./img/case.png"));
				this.hasgroup_list.add(cans.Now_state.group_list.get(i));
			}
			if(cans.Now_state.group_list.size()>0) {
				for(int i = 0 ; i < cans.Now_state.group_list.size(); i++) { // refresh dragSelect_obj
					cans.Now_state.group_list.remove(i);
				}
			}
		...
		int offset_y = p.y - this.getY();
 		for(Base_Object obj: hasgroup_list) {
			if(obj.img_type == 1) {
				...
				obj.point.setLocation(obj.getX()+offset_x,obj.getY()+offset_y);
				obj.Top.setLocation(obj.getX()+offset_x+30,obj.getY()+offset_y+10);
				obj.Bottom.setLocation(obj.getX()+offset_x+30,obj.getY()+offset_y+70);
				obj.Left.setLocation(obj.getX()+offset_x,obj.getY()+offset_y+40);
				obj.Right.setLocation(obj.getX()+60,obj.getY()+offset_y+40);
			}else if(obj.img_type == 2) {
				obj.setLocation(obj.getX()+offset_x,obj.getY()+offset_y);
				obj.point.setLocation(obj.getX()+offset_x,obj.getY()+offset_y);
				obj.Top.setLocation(obj.getX()+offset_x+35,obj.getY()+offset_y+20);
				obj.Bottom.setLocation(obj.getX()+offset_x+30,obj.getY()+offset_y+60);
				obj.Left.setLocation(obj.getX()+offset_x,obj.getY()+offset_y+40);
				obj.Right.setLocation(obj.getX()+60,obj.getY()+offset_y+40);
			}else {
				obj.setLocation(new Point(obj.getX()+offset_x,obj.getY()+offset_y));
			}
		}
 		super.setLocation(p);
	}
