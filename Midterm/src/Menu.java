...
class Group_item extends Menu_item{
	...
	public void actionPerformed(ActionEvent e) {
		...
		for(int i = 0; i < Canvas.Now_state.group_list.size() ; i++) {
			if(Max_x < Canvas.Now_state.group_list.get(i).getX()+Canvas.Now_state.group_list.get(i).getWidth()) {
			...
			if(Max_y < Canvas.Now_state.group_list.get(i).getY()+Canvas.Now_state.group_list.get(i).getHeight()) {
		...
		Base_Object composit = new group_Object(cans,Min_x,Min_y,Max_x,Max_y);
		cans.add(composit);
		cans.repaint();
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		for(int i = cans.Now_state.object_list.size()-1 ; i >= 0;i--) {
			if(cans.Now_state.object_list.get(i).isgroup) {
				for(Base_Object obj:cans.Now_state.object_list.get(i).hasgroup_list) {
					obj.ingroup = false;
				}
				cans.remove(cans.Now_state.object_list.get(i));
				cans.Now_state.object_list.remove(cans.Now_state.object_list.get(i));
				break;
			}
		}
		cans.repaint();
